...
function routes (app, database) {
  ...
  app.route('/api/login')
    ...
    .post( ensureAuthenticated,
      ...
      passport.authenticate('local', function(err, user, info) {
        ...
        if (!user) { return res.json({auth: false, msg: 'Incorrect username or password'}); }
        req.login(user, function(err) {
          if (err) { return next(err); }
          return res.json({auth: true, msg: `${req.user.username} has successfully signed in!`, username: req.user.username});
        });
      })(req, res, next);
    });
  ...
  app.route('/api/register')
      ...
      database(async function (client) {
        ...
        if ( findResults === null ) {
          ...
          if( insertResults.insertedCount === 0 ){
            res.json({auth: false, msg: 'An error occurred while trying to create a user. Please try again.'});
          ...
          res.json({auth: false, msg: 'Error, user already exists! Please pick a new name.'})
    ...
    (req, res, next) => {
      res.json({auth: true, msg: 'Account created successfully!', username: req.body.username});
